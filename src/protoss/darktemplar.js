...
export class DarkTemplar extends Infantry {
    constructor(position) {
        super(position);

        const unitData = assetManager.get('unit_darktemplar').stats;
        this.name = 'Dark Templar';
        this.portraitUrl = 'assets/images/protoss/darktemplar_portrait.png';
        ...
        /** @tweakable Shield recharge rate for Dark Templar (shields per second). */
        ...
        try {
            const asset = assetManager.get('protoss_darktemplar');
            this.mesh = this.createMeshFromGLB(asset);
        } catch (error) {
            console.warn('Could not load dark templar model, using procedural fallback.', error);
    ...
    createProceduralMesh() {
        ...
        const bodyMaterial = new THREE.MeshStandardMaterial({ color: 0x222222, metalness: 0.8, roughness: 0.4 });
        const armorMaterial = new THREE.MeshStandardMaterial({ color: 0x555555, metalness: 0.9, roughness: 0.3 });
