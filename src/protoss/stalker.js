...
export class Stalker extends Infantry {
    constructor(position) {
        super(position);

        const unitData = assetManager.get('unit_stalker').stats;
        this.name = 'Stalker';
        this.portraitUrl = 'assets/images/protoss/stalker_portrait.png';
        ...
        /** @tweakable Shield recharge rate for Stalkers (shields per second). */
        ...
        try {
            const asset = assetManager.get('protoss_stalker');
            this.mesh = this.createMeshFromGLB(asset);
        } catch (error) {
            console.warn("Could not load stalker model, using procedural fallback.", error);
    ...
    createProceduralMesh() {
        ...
        const bodyMaterial = new THREE.MeshStandardMaterial({ color: 0x333355, metalness: 0.9, roughness: 0.4 });
        const coreMaterial = new THREE.MeshStandardMaterial({ color: 0x00ffff, emissive: 0x00ffff, emissiveIntensity: 0.8 });

        const torsoGeo = new THREE.BoxGeometry(1.2, 0.8, 1.2);
